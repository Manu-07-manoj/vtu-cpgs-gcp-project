...
def compute_engine_metrics(projectid,client, aggregation,startyearname, endyearname,startmonth,endmonth,
                           ...
                           before_yearmonht, afteryearmonth,todate, agg):
    ...
    metrics = ['metric.type = "compute.googleapis.com/instance/cpu/utilization"']
    ...
    for i in range(len(metrics)):
        ...
        if (len(df_concat)> 0):
            df_concat.to_csv(f'C:/Manu/code/Python/firstproject/GCP-project/files/{projectid}_CE_metrics_{todate}_{agg}.csv' , index=False)
...
for projectid in projectids :
    compute_engine_metrics(projectid, client, average(), startyearname, endyearname, startmonth, endmonth, lastday,
                       firstday, yearmonth, dateextention, storage_client, bucketname, before_yearmonht, afteryearmonth,todate, agg="avg")
    
    compute_engine_metrics(projectid, client, maximum(), startyearname, endyearname, startmonth, endmonth, lastday,
                       firstday, yearmonth, dateextention, storage_client, bucketname, before_yearmonht, afteryearmonth, todate, agg="max")